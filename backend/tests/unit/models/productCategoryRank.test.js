...
describe(`ProductCategoryRank Model Tests`, () => {
    ...
    test('Select a valid ProductCategoryRank', async () => {
        ...
        expect(parseFloat(productCategoryRank.rankingThresholdPercentage)).toBe(expectedVatRate);
    });
    ...
    test('ProductCategoryRank hasMany Asin', async () => {
        ...
        const asins = await db.Asin.findAll({ where: { productCategoryRankId: expectedProducCategoryRankId } });
        asins.forEach(asin => {
