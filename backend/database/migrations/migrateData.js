 ...
 * @returns {Promise<Array>} - Promise resolving to array of objects representing CSV data.
 */
    ...
    try {
        ...
        data = data.map(row => {
            return Object.fromEntries(
                Object.entries(row).map(([key, value]) => [key, value.trim() === ''|| value.trim() === 'null' ? null : value.trim()])
            );
        });
